...
// La leyenda, con una muestra de cada color: en texto no cabia y se cortaba en "azul: ...".
function mapKey(matched=false){const k=document.querySelector('.nav-map-key');if(!k)return;k.innerHTML=(matched?'<span class="clave" style="--c:#d88922">Calles</span>':'')+'<span class="clave" style="--c:#526e82">Ruta</span><span class="clave" style="--c:#248660">Hecho</span><span class="clave" style="--c:#176dcc">Próximo</span>';k.setAttribute('aria-label','Leyenda del mapa. '+(matched?'Dorado: calles reconocidas. ':'')+'Gris: recorrido. Verde: hecho. Azul: siguiente tramo.');}
...
window.RutasStreetMatch={get:()=>({pts:path.slice(),active:!!layer,gaps:cortesVisibles,state:matchState,navigable,coverage,lengthPct})};
match();
})();
