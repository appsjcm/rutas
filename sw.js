const VERSION='rutas-shell-v1',TILES='rutas-tiles-v1',FONTS='rutas-fonts-v1',TILE_CAP=1500;
...
self.addEventListener('activate',e=>{e.waitUntil((async()=>{const keep=[VERSION,TILES,FONTS];for(const k of await caches.keys())if(!keep.includes(k))await caches.delete(k);await self.clients.claim();})())});
