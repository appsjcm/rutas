...
Parado -en cada contenedor, que es buena parte de una ronda- el GPS sigue dando posiciones que bailan unos metros. Antes cada baile movía el mapa con animación, casi un segundo de cada segundo; la flecha giraba hacia donde cayera el ruido o se volvía un punto, y con el mapa orientado a la marcha giraba el mapa entero. Ahora, por debajo de 3,6 km/h, la flecha guarda el último rumbo, el vehículo no se mueve mientras la posición nueva caiga dentro del margen de error del GPS y el mapa no se recoloca mientras el vehículo no se salga de ese margen en pantalla. Con el simulador detenido: con buena señal, ni un movimiento en diez segundos; con señal irregular, dos, frente a uno por segundo. La pantalla quieta es también batería. El zoom va con la velocidad -18 despacio, 17 en calle, 16 en carretera- con un margen de 3,6 km/h en cada umbral, porque circulando a 29 km/h justos cambiaba a cada posición, y el cambio se hace con animación en vez de golpe.
...
También se dicen de viva voz: una vez al acercarse -«Parada 3 de 14 en 150 m», unos veinte segundos antes y nunca a menos de 150 m ni a más de 500- y otra al llegar, con lo que no se sabe de memoria: cuánto duró esa parada en la ronda grabada («En la grabación duró 6 minutos»). Nunca pisan una indicación de giro: si hay algo sonando, esperan a la siguiente posición, y cada aviso sale una sola vez. Ya en la parada, la línea del panel pasa de «a 20 m» a «En la parada 3 de 14 · 6 min en la grabación». Las distancias se redondean como las de los giros.

El simulador se paraba al pasarse del final, con la última posición a unos 17 m del último punto a 60 km/h, y la llegada solo se reconoce a menos de 15: nunca se veía ni se oía «Has llegado». Ahora el último paso cae justo en el final.

## La voz

El navegador no elige voz: con el idioma puesto, el móvil usaba la básica aunque tuviera descargada una mejorada o premium -en iPhone, en Ajustes › Accesibilidad › Contenido leído › Voces-, que suenan mucho mejor. Ahora se elige la mejor en castellano con este orden: de España antes que de otros países; premium y mejoradas antes que la básica; y las que funcionan sin conexión antes que las que la necesitan, porque en la ronda no siempre hay cobertura y una voz en línea se quedaría muda. Las voces de broma que trae el sistema -Eddy, Flo, Grandma...- quedan fuera. En «Ruta y ajustes», debajo de «Avisos por voz», se puede escoger otra y probarla; la elección se recuerda. Toda frase que dice la aplicación -indicaciones, tramos sin pasar, marcas- sale con esa voz. La elección es pura y con pruebas (`hud-core.js`, `bestVoice`); `voz.js` solo la aplica.

## La pantalla, encendida

Si el móvil se bloquea a mitad de ronda, el GPS del navegador deja de actualizarse y se acaban las indicaciones. La aplicación pide la pantalla encendida al arrancar, pero no todos los móviles lo dejan -en iPhone, instalada en la pantalla de inicio, no funcionó hasta iOS 18.4- y el fallo era silencioso. Ahora el chequeo de salida tiene un apartado **Pantalla**: «Se mantendrá encendida durante la ronda» si el móvil lo acepta, o, si no, la advertencia con el ajuste que hay que tocar: Bloqueo automático en «Nunca», en Ajustes › Pantalla y brillo. No se pide dos veces: el chequeo espera un momento a lo que le contestaron a la navegación, que la pide en el mismo toque. Y si el sistema suelta la pantalla con la aplicación a la vista -batería baja, por ejemplo-, se vuelve a pedir; al ocultarla la suelta el sistema y se pide al volver, como antes.
...
Cuando algo falla en el móvil y no se está delante -«no se actualiza», «no hace la rotonda»-, hay que adivinar. En la Guía, bajo la versión, hay ahora un desplegable **Diagnóstico** con lo que hace falta saber: versión instalada, navegador y sistema -sin modelo ni nada que identifique a nadie-, si está instalada en la pantalla de inicio, cuánto ocupa y si está protegida del borrado, si la pantalla se pudo mantener encendida, cuántas voces en castellano hay y cuál se usa, el permiso del GPS, la conexión y los últimos errores. **Copiar diagnóstico** lo deja en el portapapeles para mandarlo; si el navegador no deja copiar, se dice que se haga una captura. No incluye nada de la ruta ni de la posición. Los errores los apunta un recogedor que va lo primero de la página, así que también quedan los del arranque; solo el mensaje y el fichero, los ocho últimos. La parte que se puede probar sin pantalla está en `diagnostico-core.js`.

## Las teselas, con su peso real

Al estrenar el diagnóstico salió que la aplicación ocupaba **367 MB** en un navegador de pruebas con 54 teselas guardadas. La razón: las teselas se pedían y se guardaban «opacas» -sin CORS- y el navegador apunta cada respuesta opaca como si pesara unos 7 MB, para no revelar su tamaño real. Con el tope de 1500 teselas serían unos 10 GB ficticios, y en Android eso puede llevar al navegador a borrar todo lo de la aplicación -rutas incluidas- para hacer sitio. Los cuatro servidores de teselas -OpenStreetMap y los tres de Esri- permiten CORS, así que ahora se piden así (`crossOrigin` en las capas de Leaflet, y también la hoja de fuentes de Google) y el service worker no guarda nunca una respuesta opaca. Resultado: **3,6 MB**. Las cachés de teselas y fuentes cambian de nombre para que las viejas se borren; el mapa sin conexión se vuelve a llenar solo al ir viéndolo, sin descargas masivas, como pide OpenStreetMap. Dos pruebas nuevas vigilan que todas las capas lleven CORS y que el chequeo de salida mire la misma caché que llena el service worker.
