...
let report=null,layer=null,token=0,controller=null,routeRef=null,acks=[],ackKey=null,profile={},lastData=null,lastCached=false;
...
function reset(){const state=RutasMap.get();if(!state.route||state.route.pts===routeRef)return;routeRef=state.route.pts;loadAcks(state.route);token++;if(controller)controller.abort();report=null;lastData=null;lastCached=false;if(layer){state.map.removeLayer(layer);layer=null;}$('road-issues').replaceChildren();$('road-export').disabled=true;if(state.mode==='access'){$('road-check').disabled=true;$('nav-road-alert').hidden=true;$('road-status').textContent='Acceso calculado para coche. Las medidas del vehículo no se tienen en cuenta en este trayecto.';return;}$('road-check').disabled=Roadbook.getRoute().sample;$('road-status').textContent=Roadbook.getRoute().sample?'Carga tu GPX real para comprobarlo.':'Sin comprobar. Pulsa Comprobar mi recorrido; se consulta únicamente la zona del mapa.';$('nav-road-alert').hidden=true;try{const cache=JSON.parse(localStorage.getItem(key(routeRef)));if(cache&&Date.now()-cache.saved<86400000)render(cache.data,state.route,true);}catch{}}
const KINDS={opposed:'Posible sentido contrario',conditional:'Sentido variable: revisar condiciones',ambiguous:'Coincidencia dudosa entre vías cercanas',height:'El vehículo no pasa por altura',weight:'El vehículo supera el peso permitido',width:'El vehículo no pasa por anchura',access:'Vía cerrada a este vehículo','limit-variable':'Límite dimensional variable: revisar condiciones'};
...
function alertAt(d){const issue=upcoming(d);$('nav-road-alert').hidden=!issue;if(issue)$('nav-road-alert').textContent=(HARD.has(issue.kind)?'⛔ ':'⚠ ')+description(issue)+' · '+issue.name+(issue.start>d?' · en '+Math.round(issue.start-d)+' m':'')+'. No sigas esta traza sin comprobar la señalización.';}
window.RutasChecks={getIssues:()=>report?report.issues.map(i=>({p:i.p,name:i.name,description:description(i),muted:muted(i),hard:HARD.has(i.kind)})):[],titleAt:d=>{const i=upcoming(d);return i?description(i):null;},gpsAlert:d=>{const i=upcoming(d);return i?'Atención. '+description(i)+'. '+i.name+'. Revisa la señalización antes de continuar.':null;}};
