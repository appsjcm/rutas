...
window.RutasChecks={getIssues:()=>report?report.issues.map(i=>({p:i.p,name:i.name,description:description(i),muted:muted(i),hard:HARD.has(i.kind)})):[],roadAt,titleAt:d=>alertInfo(d)?.title||null,alertInfo,gpsAlert:d=>alertInfo(d)?.voice||null};
