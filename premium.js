...
body.prepend(hero);
function overview(){const data=window.Roadbook.getRoute();hero.classList.toggle('has-route',!data.sample);hero.querySelector('h2').textContent=data.sample?'Cada calle, a tu ritmo.':String(data.name||'Mi recorrido').replace(/\.gpx$/i,'');hero.querySelector('p').textContent=data.sample?'Carga una ruta para empezar. Mientras tanto, explora el recorrido de ejemplo.':'Tu recorrido está preparado. Revisa los detalles y sal cuando quieras.';hero.querySelector('.route-private').textContent=data.sample?'Vista de ejemplo':'Solo en tu dispositivo';const route=window.RutasNavCore?window.RutasNavCore.prepare(data.pts):window.RutasMap?.get().route;const facts=hero.querySelector('.route-facts');facts.replaceChildren();if(route){const length=document.createElement('span');length.textContent=(route.total/1000).toLocaleString('es-ES',{maximumFractionDigits:1})+' km';length.prepend(icon('route'));facts.append(length);}const segment=document.createElement('span');segment.textContent=data.tracks.length===1?'1 segmento':data.tracks.length+' segmentos';segment.prepend(icon('map'));facts.append(segment);row.hidden=data.tracks.length<2;document.getElementById('nav-name').hidden=true;}
