...
function match(r,p,progress,forward=200){let best=null;const kx=111320*Math.cos(p.lat*rad),ky=110540;for(let i=0;i<r.pts.length-1;i++){if(r.cum[i+1]<progress-30||r.cum[i]>progress+forward)continue;const a=r.pts[i],b=r.pts[i+1],x=(a.lon-p.lon)*kx,y=(a.lat-p.lat)*ky,dx=(b.lon-a.lon)*kx,dy=(b.lat-a.lat)*ky;const t=Math.max(0,Math.min(1,-(x*dx+y*dy)/(dx*dx+dy*dy||1)));const d=r.cum[i]+(r.cum[i+1]-r.cum[i])*t;if(d<progress-30||d>progress+forward)continue;const error=Math.hypot(x+t*dx,y+t*dy),score=error+Math.abs(d-progress)*0.03;if(!best||score<best.score)best={d,error,score};}return best;}
const api={distance,prepare,at,heading,match};if(typeof module!=='undefined')module.exports=api;else root.RutasNav=api;
