...
function speedAt(r,d){const k=indexAt(r,d),i=Math.max(0,k-8),j=Math.min(r.pts.length-1,k+8),a=Date.parse(r.pts[i]?.time),b=Date.parse(r.pts[j]?.time);if(!Number.isFinite(a)||!Number.isFinite(b)||b===a)return NaN;return (r.cum[j]-r.cum[i])/(Math.abs(b-a)/1000)*3.6;}
...
function decodeSigned(str,c){let r=0,shift=0,b;do{if(c.i>=str.length)throw Error('Enlace incompleto.');b=str.charCodeAt(c.i++)-63;if(b<0||b>63||shift>=35)throw Error('El enlace contiene datos inválidos.');r|=(b&0x1f)<<shift;shift+=5;}while(b>=0x20);return (r&1)?~(r>>1):(r>>1);}
...
function remainingSeconds(r,d){if(d>=r.total)return 0;const last=r.pts.length-1,t0=Date.parse(r.pts[0]?.time),tN=Date.parse(r.pts[last]?.time);if(!Number.isFinite(t0)||!Number.isFinite(tN)||tN===t0)return null;const tk=Date.parse(r.pts[indexAt(r,Math.max(0,Math.min(r.total,d)))]?.time);return Number.isFinite(tk)?Math.abs(tN-tk)/1000:null;}
