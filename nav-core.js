...
function turns(r){const candidates=[];for(let d=25;d<r.total-25;d+=5){const a=at(r,d-25),b=at(r,d),c=at(r,d+25);if(distance(a,b)<10||distance(b,c)<10)continue;const angle=((heading(b,c)-heading(a,b)+540)%360)-180;if(Math.abs(angle)<48)continue;const item={d,angle,label:Math.abs(angle)>135?'Cambio de sentido':angle>0?'Giro a la derecha':'Giro a la izquierda',symbol:Math.abs(angle)>135?'↶':angle>0?'↱':'↰'};const last=candidates.at(-1);if(last&&d-last.end<40&&(Math.sign(last.angle)===Math.sign(angle)||(Math.abs(last.angle)>135&&Math.abs(angle)>135))){last.end=d;if(Math.abs(angle)>Math.abs(last.angle))Object.assign(last,item);}else candidates.push({...item,end:d});}return candidates;}
...
const api={distance,prepare,at,heading,match,section,turns,guidance,fingerprint,nearbyPasses};if(typeof module!=='undefined')module.exports=api;else root.RutasNav=api;
